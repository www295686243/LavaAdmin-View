...
class Service extends BaseAbstract {
  displayName = '提现'
  name = 'UserCash'
  controllerName = 'UserCashApplyController'
  index (params: { page: number }) {
    return axios.get('user_cash_apply', {
      ...params,
  ...
  update (form: { id: string; status: number }) {
    return axios.put(`user_cash_apply/${form.id}`, form)
  }

  show (id: string) {
    return axios.get(`user_cash_apply/${id}`)
  }
}
